...
namespace WM.UnitTestScribe.Summary {
    public class TestCaseAnalyzer {
        ...
        public VariableAssignmentManager VAssignmentManager { get; private set; }
        ...
        private VariableDeclaration FindCheckingObjectVarDeclariation(List<Expression> subExps) {
            foreach (var exp in subExps) {
                if (exp is MethodCall) {
                    var call = (MethodCall)exp;
                    ...
                    if (arguments.Count > 0) {
                        ...
                        if (call.Name.StartsWith("assert")|| call.Name == "IsTrue" || call.Name == "IsFalse" || call.Name == "IsNotNull" ||
                            ...
                            if (!expArg.Components.Any()) {
                                var argVarUse = expArg as NameUse;
                                if (argVarUse != null) {
                                    var vd = argVarUse.FindMatches().FirstOrDefault() as VariableDeclaration;
                                    if (vd != null)
                                        return vd;
                                }
                            }
                            int tempPos = 0;
                            var argExps = expArg.Components.ToList();
                            while (tempPos <= argExps.Count() - 1) {
                                var argVarUse = FindAndUpdateNextVarUse(argExps, tempPos, out tempPos, false);
                                if (argVarUse != null) {
                                    var vd = argVarUse.FindMatches().FirstOrDefault() as VariableDeclaration;
                                    if (vd != null)
                                        return vd;
                            ...
                            //The last argument
                            var expArg = arguments.ElementAt(arguments.Count - 1);
                            if (arguments.Count == 3) {
                                expArg = arguments.ElementAt(1);
                            }
                            ...
                            while (tempPos <= argExps.Count() - 1) {
                                ...
                                if (argVarUse != null) {
                                    ...
                                    var allMethodCall = from expression in expArg.GetDescendantsAndSelf<MethodCall>()
                                                          select expression; 
                                    //var exp = argExps.ElementAt(tempPos - 1);
                                    foreach (var innerCall in allMethodCall) {
                                        var args = innerCall.Arguments.ToList();
                                        foreach (var arg in args) {
                                            int tempPos2 = 0;
                                            if (!arg.Components.Any()) {
                                                var argVarUse2 = arg as NameUse;
                                                if (argVarUse2 != null) {
                                                    var vd = argVarUse2.FindMatches().FirstOrDefault() as VariableDeclaration;
                                                    if (vd != null)
                                                        return vd;
                                                }
                                                continue;
                                            ...
                                            var argExps2 = arg.Components.ToList();
                                            int i = 1;  // no dead loop
                                            while (tempPos2 <= argExps.Count() - 1 && i < 10) {
                                                i++;
                                                var argVarUse2 = FindAndUpdateNextVarUse(argExps2, tempPos2, out tempPos2, false);
                                                if (argVarUse2 != null) {
                                                    var vd = argVarUse2.FindMatches().FirstOrDefault() as VariableDeclaration;
                                                    if (vd != null)
                                                        return vd;
        ...
        private bool HasMethodCallOnRight = false;
        private HashSet<VariableDeclaration> ArgVarAssigning = new HashSet<VariableDeclaration>();
                ...
                if (opExp != null) {
                    ...
                    if (opExp.Text == "=" || opExp.Text == "+=" || opExp.Text == "-=") {
                        ...
                        if (subExps.ElementAt(curPos - 1) is NameUse && !subExps.ElementAt(curPos - 1).GetType().IsSubclassOf(typeof(NameUse))) {
                            ...
                            VariableDeclaration vd = assignedSelfField;
                            if (vd == null && varInfo != null) {
                                vd = varInfo.Variable;
                            }
                            curPos = UpdateAssignmentExpRightHand(subExps, curPos + 1, vd, false);
                            ...
                            //varInfo not null
                            curPos = UpdateAssignmentExpRightHand(subExps, curPos, vd, IsFieldUpdate);
        ...
        private NameUse FindAndUpdateNextVarUse(List<Expression> subExps, int startPos, out int newPos, bool IsUpdate) {
            ...
            for (int curPos = startPos; curPos <= subExps.Count() - 1; curPos++) {
                ...
                //    continue;
                //}
                if (curExp.Components.Any()) {
                    int tempPos = 0;


                    var tempExps = curExp.Components.ToList();
                    while (tempPos <= tempExps.Count() - 1) {
                        var argVarUse = FindAndUpdateNextVarUse(tempExps, tempPos, out tempPos, false);
                        if (argVarUse != null) {
                            newPos = curPos + 1;
                            return argVarUse;
                        }
                ...
                if (curExp is MethodCall) {
                    HasMethodCallOnRight = true;
                    MethodCall mc = (MethodCall)curExp;
                    ...
                    foreach (var arg in args) {
                        int tempPos = 0;
                        if (!arg.Components.Any()) {
                            var argVarUse = arg as NameUse;
                            if (argVarUse != null) {
                                var vd = argVarUse.FindMatches().FirstOrDefault() as VariableDeclaration;
                                if (vd != null)
                                    ArgVarAssigning.Add(vd);
                            }
                            continue;
                        }

                        var argExps = arg.Components.ToList();
                        while (tempPos <= argExps.Count() - 1) {
                            var argVarUse = FindAndUpdateNextVarUse(argExps, tempPos, out tempPos, false);
                            if (argVarUse != null) {
                                var vd = argVarUse.FindMatches().FirstOrDefault() as VariableDeclaration;
